...
class Trajectory(object):
    ...
    def trajectoryGraph(self,displayGraph,plot3D = False,excelFile = False):
        ...
        self.dist_x_initial = 0
        self.velocity_x_initial = self.V * math.cos(self.elevation_angle)+ self.crosswind*(-1)*math.sin(self.windDirection*0.0174533)
        self.dist_z_initial = 0
        self.velocity_z_initial = self.V * math.sin(self.elevation_angle)

        velocity_x_instant = self.velocity_x_initial
        dist_x_instant = self.dist_x_initial
        ...
        for t in range(0,self.steps+1):
            ...
            dist_y_instant = dist_x_instant*self.windage_angle
            wind_deflection = (-1) * self.windValue * (t * self.time_interval - dist_x_instant / self.V)
            ...
            velocity_z_instant = -9.81*currentTime + self.velocity_z_initial
            dist_z_instant = -4.905*pow(currentTime,2) + self.velocity_z_initial*currentTime + self.dist_z_initial

            self.distance_graph.append(dist_x_instant_updated)
            self.elevation_graph.append(dist_z_instant)
            self.velocity_graph.append(velocity_x_instant_updated)
        ...
        if(displayGraph == True):
            ...
            #Velocity in fps
            velocity_graph = [x * 3.28084 for x in self.velocity_graph]
